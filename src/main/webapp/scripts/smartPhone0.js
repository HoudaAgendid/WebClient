$(document).ready(function() {
	...
	$("#content").on("click", '.btn', function() {
		...
		} else if ($(this).attr("data-role") === "map0") {
			var id = $(this).attr('indice');
			//alert(id);
			if (id != undefined && id != null) {
				window.location = '/WebGeo/map0.html?id=' + id;
			}
		} else {
			$.ajax({
				url: 'SmartPhoneServlet',
	...
	function listerSpc(data) {
		...
		data.forEach(function(e) {
			ligne += "<tr><th scope='row'>" + e.id + "</th><td value='" + e.user.nom + "' >" + e.user.nom + "</td><td valuel=" + e.imei + ">" + e.imei
				+ "</td><td><button indice=" + e.id + " data-role='delete' class='btn btn-danger'>Delete</button>&nbsp;&nbsp;&nbsp;<button indice=" + e.id
				+ " data-role='update' class='btn btn-success'>Update</button>&nbsp;&nbsp;&nbsp;<button indice=" + e.id
				+ " data-role='map0' class='btn btn-info'>Map</button></td></tr>";
		});
		$("#content").html(ligne);
	}
});
